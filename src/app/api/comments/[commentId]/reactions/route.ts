...
async function getReactionsHandler(req: AuthenticatedRequest, context: RouteContext) {
  ...
  try {
    ...
    const commentBoardResult = await query(
      `SELECT c.post_id, p.board_id, b.settings, b.community_id
       FROM comments c 
    ...
    if (!resolvedBoard) {
      console.warn(`[API GET /api/comments/${commentId}/reactions] User ${userId} from community ${userCommunityId} attempted to access comment from inaccessible board ${board_id}`);
      return NextResponse.json({ error: 'Comment not found' }, { status: 404 });
    }
    ...
    if (!canUserAccessBoard(userRoles, boardSettings, isAdmin)) {
      console.warn(`[API GET /api/comments/${commentId}/reactions] User ${userId} attempted to access reactions on restricted board ${board_id}`);
...
async function toggleReactionHandler(req: AuthenticatedRequest, context: RouteContext) {
  ...
  try {
    // SECURITY: First, check if user can access the board where this comment belongs
    const commentBoardResult = await query(
      `SELECT c.post_id, c.content as comment_content, p.title as post_title, p.board_id, b.settings, b.community_id, b.name as board_name
       FROM comments c 
       JOIN posts p ON c.post_id = p.id
       JOIN boards b ON p.board_id = b.id 
       WHERE c.id = $1`,
    ...
    const { post_id, comment_content, post_title, board_id, settings, board_name } = commentBoardResult.rows[0];
    
    // Verify user can access the board (handles both owned and shared boards)
    const resolvedBoard = await resolveBoard(board_id, userCommunityId || '');
    if (!resolvedBoard) {
      console.warn(`[API POST /api/comments/${commentId}/reactions] User ${userId} from community ${userCommunityId} attempted to react to comment from inaccessible board ${board_id}`);
      return NextResponse.json({ error: 'Comment not found' }, { status: 404 });
    }
    ...
    // Check board access permissions
    if (!canUserAccessBoard(userRoles, boardSettings, isAdmin)) {
      console.warn(`[API POST /api/comments/${commentId}/reactions] User ${userId} attempted to react on restricted board ${board_id}`);
    ...
    if (boardLockGating && boardLockGating.lockIds.length > 0) {
      console.log(`[API POST /api/comments/${commentId}/reactions] Board ${board_id} has ${boardLockGating.lockIds.length} lock requirements, checking user verification...`);
      ...
      if (!hasAccess) {
        console.log(`[API POST /api/comments/${commentId}/reactions] User ${userId} failed board lock verification: ${verifiedCount}/${requiredCount} locks verified (${boardLockGating.fulfillment} mode)`);
        return NextResponse.json({ 
      ...
      console.log(`[API POST /api/comments/${commentId}/reactions] ✅ User ${userId} passed board lock verification: ${verifiedCount}/${requiredCount} locks verified`);
    }
    ...
    console.log(`[API /api/comments/${commentId}/reactions POST] Attempting to use process.customEventEmitter. Emitter available:`, !!emitter);
    if (emitter && typeof emitter.emit === 'function') {
      // Create a preview of the comment content for the event
      const commentPreview = comment_content ? comment_content.substring(0, 100) : '';
      ...
      console.log(`[API /api/comments/${commentId}/reactions POST] Successfully emitted commentReactionUpdate event for ${action} ${cleanEmoji}.`);
    } else {
      console.error(`[API /api/comments/${commentId}/reactions POST] ERROR: process.customEventEmitter not available.`);
    }
